...
static inline void insertSkip(int idx) {
    auto it = next(brackets.begin(), idx);
    skip.insert(it->first);
...
static inline void eraseSkip(int idx) {
    auto it = next(brackets.begin(), idx);
    skip.erase(it->first);
    skip.erase(it->second);
}

// 현재 backtrack2()의 깊이 제한
int depth_limit;
...
void backtrack2(int curr_depth, int start) {
    if (curr_depth == depth_limit) {
        ...
        for (int i = start; i < brackets_size; i++) {
            ...
            backtrack2(curr_depth + 1, i + 1); // 백트랙 인자 실수하지 말자 제발..
